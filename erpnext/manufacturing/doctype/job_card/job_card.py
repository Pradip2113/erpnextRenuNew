...
# ---------------------- Start Custom Code 
	...
	def insert_result_todb(self,row_index):
		if(self.pcd_counter==4):
			server = '103.75.60.120'  
			user = 'sa'  
			password = 'Admin@123$'
			database = 'ERPAdmin'  # Corrected database name
			...
			try:
				...
				qr_code =self.get("time_logs")[row_index-1].scan_qr
				product_type =self.get("time_logs")[row_index-1].product_type
				if qr_code:
					# Check if qr_code exists
					cursor.execute("SELECT COUNT(*) FROM JobCard WHERE qr_code = %s", (qr_code,))
					count = cursor.fetchone()[0]

					if count > 0:
						# If qr_code exists, check if product_type needs to be updated
						cursor.execute("SELECT COUNT(*) FROM JobCard WHERE product_type = %s AND qr_code = %s", (product_type, qr_code))
						count_product_type = cursor.fetchone()[0]
						
						if count_product_type == 0:
							# If product_type does not match, update it
							cursor.execute("UPDATE JobCard SET product_type = %s WHERE qr_code = %s", (product_type, qr_code))
							conn.commit()
						else:
							pass
					else:
						# If qr_code does not exist, insert a new record
						cursor.execute("INSERT INTO JobCard (qr_code, product_type,work_order) VALUES (%s, %s, %s)", (qr_code, product_type,self.name))
						conn.commit()
			except pymssql.Error as e:
				frappe.throw("Error connecting to SQL Server:", e)

			finally:
				if 'conn' in locals():
					conn.close()
	#This method is uded to get qr item from work order
	@frappe.whitelist()
	def pcb(self):
		if(self.pcd_counter == 0 and self.counter==1):
			doc1 = frappe.get_doc('Work Order', self.work_order)
			self.shift_incharge = doc1.shift_incharge
			self.incharge_name = doc1.incharge_name
			self.shift = doc1.shift
			
			if doc1.pcb_details:
				for detail in doc1.pcb_details:
					self.append("time_logs",{
						"pcb_qr_code":detail.qr_code,
						"completed_qty":1,
						"from_time":detail.current_time,
						# "scan_qr_start":detail.qr_code
					})
			else:
				frappe.msgprint("No PCB details found in the Work Order.")
		self.pcd_counter = 1

			
	#This function to append qr data to child table and check the qr available or not 
	@frappe.whitelist()
	def append_data_qrend(self):
		if(self.pcb_code):
			flag = 0
			msg =0
			for index in self.get("time_logs"):
				if(str(self.pcb_code)==str(index.pcb_qr_code)):
					msg+=1
					flag +=1
					if(index.scan_qr==None or index.to_time==None):
						index.set("scan_qr", self.pcb_code)
						result = datetime.now()
						index.set("to_time",result)
						self.get_subprocess_data(self.pcb_code)
						if(len(self.get("time_logs"))>0):
							templ=self.get("time_logs")[0].scan_qr
							self.get("time_logs")[0].scan_qr=self.get("time_logs")[index.idx-1].scan_qr
							self.get("time_logs")[index.idx-1].scan_qr=templ
							
							templ=self.get("time_logs")[0].pcb_qr_code
							self.get("time_logs")[0].pcb_qr_code=self.get("time_logs")[index.idx-1].pcb_qr_code
							self.get("time_logs")[index.idx-1].pcb_qr_code=templ

							templ=self.get("time_logs")[0].to_time
							self.get("time_logs")[0].to_time=self.get("time_logs")[index.idx-1].to_time
							self.get("time_logs")[index.idx-1].to_time=templ
							
							templ=self.get("time_logs")[0].pcb_test_results
							self.get("time_logs")[0].pcb_test_results=self.get("time_logs")[index.idx-1].pcb_test_results
							self.get("time_logs")[index.idx-1].pcb_test_results=templ

							templ=self.get("time_logs")[0].user
							self.get("time_logs")[0].user=self.get("time_logs")[index.idx-1].user
							self.get("time_logs")[index.idx-1].user=templ

							templ=self.get("time_logs")[0].time_in_mins
							self.get("time_logs")[0].time_in_mins=self.get("time_logs")[index.idx-1].time_in_mins
							self.get("time_logs")[index.idx-1].time_in_mins=templ

							templ=self.get("time_logs")[0].completed_qty
							self.get("time_logs")[0].completed_qty=self.get("time_logs")[index.idx-1].completed_qty
							self.get("time_logs")[index.idx-1].completed_qty=templ

							templ=self.get("time_logs")[0].scan_qr_start
							self.get("time_logs")[0].scan_qr_start=self.get("time_logs")[index.idx-1].scan_qr_start
							self.get("time_logs")[index.idx-1].scan_qr_start=templ

							templ=self.get("time_logs")[0].description
							self.get("time_logs")[0].description=self.get("time_logs")[index.idx-1].description
							self.get("time_logs")[index.idx-1].description=templ

							templ=self.get("time_logs")[0].from_time
							self.get("time_logs")[0].from_time=self.get("time_logs")[index.idx-1].from_time
							self.get("time_logs")[index.idx-1].from_time=templ
							
							templ=self.get("time_logs")[0].pre_stage_time
							self.get("time_logs")[0].pre_stage_time=self.get("time_logs")[index.idx-1].pre_stage_time
							self.get("time_logs")[index.idx-1].pre_stage_time=templ

							templ=self.get("time_logs")[0].time_diff
							self.get("time_logs")[0].time_diff=self.get("time_logs")[index.idx-1].time_diff
							self.get("time_logs")[index.idx-1].time_diff=templ
							
							templ=self.get("time_logs")[0].product_type
							self.get("time_logs")[0].product_type=self.get("time_logs")[index.idx-1].product_type
							self.get("time_logs")[index.idx-1].product_type=templ

							templ=self.get("time_logs")[0].honeywell_qr
							self.get("time_logs")[0].honeywell_qr=self.get("time_logs")[index.idx-1].honeywell_qr
							self.get("time_logs")[index.idx-1].honeywell_qr=templ
					else:
	...
	#This function is used to get the record of previously scaned qr time
	@frappe.whitelist()
	def get_prev_stage_time(self):
		if(self.counter!='1'):
			doc = frappe.get_all('Job Card', filters={"work_order": self.work_order, "counter":['<',self.counter],"name": ("!=", self.name)}, order_by="counter desc",fields=["name","counter"],limit=1)
			if(doc):
				child_doc=frappe.get_all('Job Card Time Log', filters={"parent":str(doc[0].name),"product_type":"Accept"},fields=["from_time","to_time","pcb_qr_code","product_type","pre_stage_time","honeywell_qr"])
				if(child_doc):
					chid_len=len(self.get("time_logs"))
	...
	@frappe.whitelist()
	def get_prev_stage_data(self):
		if(self.counter!='1'):
			doc = frappe.get_all('Job Card', filters={"work_order": self.work_order, "counter":['<',self.counter],"name": ("!=", self.name)}, order_by="counter desc",fields=["name","counter"],limit=1)
			if(doc):
				child_doc=frappe.get_all('Job Card Time Log', filters={"parent":str(doc[0].name),"product_type":"Accept"},fields=["from_time","to_time","pcb_qr_code","product_type","pre_stage_time"])
				if(child_doc):
					if(len(self.get("time_logs"))!=0):
						for row in child_doc:
							if(row.pcb_qr_code and row.to_time):
	...
	#Below function is used to get sub process into child table on_work_use
	def get_subprocess_data(self,pcb_code):
		existting_opr=set((i.qr_id, i.sub_operation) for i in self.get("on_work_use"))
		for i in self.get("sub_operations"):
			if((pcb_code,i.sub_operation) not in existting_opr):
				self.append("on_work_use",{"qr_id":pcb_code,"sub_operation":i.sub_operation})

	#Below function is used to get sub process into child table sub_process_details	
	# @frappe.whitelist()
	# def append_to_subprocess(self):
	# 	existing_subprocess_set = set((j.qr_id, j.sub_operation) for j in self.get("sub_process_details"))
	# 	sub_process_li=[]
	# 	for j in self.get("on_work_use"):
	# 		sub_process_li.append({
	# 			"qr_id":j.qr_id,
	# 			"sub_operation":j.sub_operation,
	# 			"result_description":j.result_description
	# 		})
	# 	qr_code=""
	# 	sub_oper=""
	# 	for k in self.get("on_work_use"):
	# 		if k.result_description and (k.qr_id, k.sub_operation) not in existing_subprocess_set:
	# 			new_data = {
	# 				"qr_id": k.qr_id,
	# 				"sub_operation": k.sub_operation,
	# 				"result_description": k.result_description
	# 			}
	# 			subprocess_details = self.get("sub_process_details")
	# 			subprocess_details.insert(0, new_data)
	# 			self.set("sub_process_details", subprocess_details)
	# 			qr_code=k.qr_id
	# 			sub_oper=k.sub_operation
	# 	self.get("on_work_use").clear()
	# 	for m in sub_process_li:
	# 		if m["qr_id"]==qr_code and m["sub_operation"]==sub_oper:
	# 			pass
	# 		else:
	# 			self.append("on_work_use"
	# 			,{
	# 				"qr_id":m["qr_id"],
	# 				"sub_operation":m["sub_operation"],
	# 				"result_description":m["result_description"],
	# 			})
	
	@frappe.whitelist()
	def append_to_subprocess_details(self):
		existing_subprocess_set = set((j.qr_id, j.sub_operation) for j in self.get("sub_process_details"))
		sub_process_li=[]
		for j in self.get("on_work_use"):
			sub_process_li.append({
		...
		for k in self.get("on_work_use"):
			if k.result_description and (k.qr_id, k.sub_operation) not in existing_subprocess_set:
				...
				subprocess_details.insert(0, new_data)
				self.set("sub_process_details", subprocess_details)
				appended_set.add((k.qr_id,k.sub_operation))
		self.get("on_work_use").clear()
		for m in sub_process_li:
			if (m["qr_id"], m["sub_operation"]) in appended_set:
				pass
			else:
				self.append("on_work_use"
				,{
	...
	def before_save(self):
		...
		for i in self.time_logs:
			if i.scan_qr_start == None or i.scan_qr_start =="":
				intot = 1 + intot
			else:
				outtot = 1 + outtot
			if i.product_type == "Accept":
				accqty = 1 + accqty
			if i.product_type == "NG":
				ngqty = 1 + ngqty
		# self.complete_qty = len(self.time_logs)
		self.complete_qty = outtot
		self.pending_qty = intot
		self.accept_qty = accqty
		self.ng_qty = ngqty

		outendtot = 0
		intendot = 0
		for i in self.time_logs:
			if i.scan_qr == None or i.scan_qr =="":
				intendot = 1 + intendot
			else:
				outendtot = 1 + outendtot
		self.complete_endqty = outendtot
		self.pending_start_qty = intendot
		self.get_prev_stage_time()
		# ------------- Custom Code pradip ------------------
...
@frappe.whitelist()
def sqldataet():
	frappe.msgprint("SQLDATA")
	server_address = 'DESKTOP-0M9RCMK\SQLEXPRESS'  # Note the double backslashes for the server address
	username = 'sa'  # Modify this if using SQL Server authentication
	password = 'admin'
	try:
		...
		sql = "INSERT INTO STUDENTTABLE (student, age, address) VALUES (?, ?, ?)"
		values = ("Soham", 555, "sangli")  # Assuming 'age' is an integer
