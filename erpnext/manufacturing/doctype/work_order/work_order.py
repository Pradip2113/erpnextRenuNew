...
class WorkOrder(Document):
	...
	# def set_qty_item(self):
	# 	size=len(self.pcb_details)
	# 	self.pcb_total=size
	# 	doc = frappe.get_all("Item", filters={"limit_qty": [">", 0]}, fields=["used_qty", "name", "limit_qty"])
	# 	for i in doc:
	# 		for j in self.get("required_items"):
	# 			if j.item_code==i.name:
	# 				total=i.used_qty+float(self.pcb_total)
	# 				if(total<=i.limit_qty):
	# 					frappe.db.set_value('Item', i.name,"used_qty",total)
	# 					k = i.limit_qty - total
	# 					frappe.db.set_value('Item', i.name,"available_qty",k)
	# 				else:
	# 					frappe.throw(f"Your Used Quantity for {i.name} is exceed than its limits")
	def set_qty_item(self):
		size=len(self.pcb_details)
		self.pcb_total=size
		doc = frappe.get_all("Item", filters={"limit_qty": [">", 0]}, fields=["used_qty", "name", "limit_qty"])
	...
	@frappe.whitelist()
	def already_exist(self,row_index):
		for i in self.get("pcb_details"):
			if(i.idx-1!=row_index):
				if(str(i.qr_code)==str(self.get("pcb_details")[row_index].qr_code)):
					frappe.throw("QR Code Already Scanned")
					self.reload()
	
	@frappe.whitelist()
	def set_qty_single(self):
		assemble = frappe.get_value("BOM",self.bom_no,'assemble_qty')
		doc= frappe.get_all('BOM Item',{'parent':self.bom_no},['item_code','qty','is_bulk_use','uom',])
		if doc:
			for k in doc:
				for m in self.get("required_items"):
					if m.item_code == k.item_code and k.is_bulk_use:
						tot = self.qty / assemble
						tot = int(tot) + (1 if tot % 1 != 0 else 0)
						frappe.db.set_value("Work Order Item", m.name,'required_qty',tot)
					if m.item_code == k.item_code:
						frappe.db.set_value("Work Order Item", m.name,'uom',k.uom)
		self.reload()

	@frappe.whitelist()
	def barcodescn_set(self):
		for m in self.get('pcb_details'):
			if (self.child_idx == m.qr_code):
				frappe.throw("QR Code Already Scanned")
		if (self.child_idx >= self.qty):
			frappe.throw("QR Code Completed")
		self.get('pcb_details')[self.child_idx].qr_code=self.barcodescn
		self.get('pcb_details')[self.child_idx].current_time= frappe.utils.now()
		self.get('pcb_details')[self.child_idx].work_order_pcb_date= frappe.utils.now()
		if(self.barcodescn == None or self.barcodescn == "" ):
			pass
		else:
			self.child_idx =self.child_idx +1
		self.barcodescn = None
		
	# @frappe.whitelist()
	# def check_ducplicate_value(self):
		# for i in self.get("pcb_details"):
		# 	# if(i!=row_index):
			# 	if(str(i.qr_code)==str(self.get("pcb_details")[row_index].qr_code)):
			# 		# self.get("pcb_details")[row_index].qr_code=None
			# 		# frappe.throw("QR Code Already Scanned")
		# frappe.throw("hi")
				# break
